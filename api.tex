...
\sloppy
\begin{itemize}
\item add \verb!$GATE_HOME/bin/gate.jar! and the JAR files in 
\verb!$GATE_HOME/lib! to the Java \texttt{CLASSPATH} 
(\verb!$GATE_HOME! is the GATE root directory)
\item tell Java that the GATE Unicode Kit is an extension: \linebreak
\verb!-Djava.ext.dirs=$GATE_HOME/lib/ext!\linebreak
N.B. This is only necessary for GUI applications that need to support Unicode
text input; other applications such as command line or web applications don't
generally need GUK.
...
All CREOLE resources have some associated meta-data in the form of an entry in a
special XML file named {\tt creole.xml}. The most important role of that
...
are. The valid parameters for a resource are described in the resource's section
of its {\tt creole.xml} file or in Java annotations on the resource class -- see
Section \ref{sec:creole-model:config}.
...
an exception. In this case, all the information needed to create a tagger is
available in default values given in the tagger's XML definition (in
{\tt plugins/ANNIE/creole.xml}):
%
\begin{small}\begin{verbatim}
<RESOURCE>
...
\end{verbatim}\end{small}
%
Here the two parameters shown are either `runtime' parameters, which are set
before a PR is executed, or have a default value (in this case the default
rules file is distributed with GATE itself).
...
\textbf{Method} & \textbf{Purpose}\\
\hline
{\tt public static void {\bf addKnownPlugin}(URL pluginURL)} & adds the plugin
to the list of known plugins.\\
\hline
{\tt public static void {\bf removeKnownPlugin}(URL pluginURL)} & tells the
system to `forget' about one previously known directory. If the specified directory 
was loaded, it will be unloaded as well - i.e. all the metadata relating to 
resources defined by this directory will be removed from memory.\\
\hline
{\tt public static void {\bf addAutoloadPlugin}(URL pluginUrl)} & adds a new 
directory to the list of plugins that are loaded automatically at start-up.\\
\hline
{\tt public static void {\bf removeAutoloadPlugin}(URL pluginURL)} & tells the 
system to remove a plugin URL from the list of plugins that are loaded 
...
\hline
\hline
{\tt public void {\bf registerDirectories}(URL directoryUrl)} & loads a new
CREOLE directory. The new plugin is added to the list of known plugins if not already
there.\\
\hline
{\tt public void {\bf registerComponent}(Class<? extends Resource> cls)} &
registers a single {\tt @CreoleResource} annotated class without the need for a
{\tt creole.xml} file.\\
\hline
{\tt public void {\bf removeDirectory}(URL directory)} & unloads a loaded CREOLE
plugin.\\
\hline
\end{tabular}
...
that will only be used from your embedded code (and so does not need to be
distributed as a complete plugin), and all the configuration for that resource
is provided as Java annotations on the class, then it is possible to register
the class with the {\tt CreoleRegister} at runtime without needing to package
it in a JAR and provide a {\tt creole.xml} file.  You can pass the {\tt Class}
object representing your resource class to
{\tt Gate.getCreoleRegister().registerComponent()} method and then create
instances of the resource in the usual way using {\tt Factory.createResource}.
Note that resources cannot be registered this way in the developer GUI, and
...
\end{itemize}
%
GATE Developer helps you with this process by creating a
set of directories and files that implement a basic resource, including a
Java code file and a Makefile. This process is called `bootstrapping'.

For example, let's create a new component called GoldFish, which will be a
Processing Resource that looks for all instances of the word `fish' in a
document and adds an annotation of type `GoldFish'.

First start GATE Developer (see Section \ref{sec:gettingstarted:install}). From
the `Tools' \begin{figure}[!htb]
\begin{center}
\includegraphics[scale=0.7]{bootstrap1.png}
\end{center}
\caption{BootStrap Wizard Dialogue}
\label{fig:bootstrap1}
\end{figure}
menu select `BootStrap Wizard', which will pop up the dialogue in figure
\ref{fig:bootstrap1}. The meaning of the data entry fields:
%
\begin{itemize}
%
\item
The `resource name' will be displayed when GATE Developer loads the
resource, and will be the name of the directory the resource lives in.
For our example: {\tt GoldFish}.
%
\item
`Resource package' is the Java package that the class representing the resource
will be created in.
For our example: {\tt sheffield.creole.example}.
%
\item
`Resource type' must be one of Language, Processing or Visual Resource. In this
case we're going to process documents (and add annotations to them), so we
select {\tt ProcessingResource}.
%
\item
`Implementing class name' is the name of the Java class that represents the
resource.
For our example: {\tt GoldFish}.
%
\item
The `interfaces implemented' field allows you to add other interfaces (e.g.
\texttt{\htlink{http://gate.ac.uk/gate/doc/javadoc/gate/creole/ControllerAwarePR.html}{gate.creole.ControllerAwarePR}}\footnote{See
Section~\ref{sec:creole-model:applications}.})
that you would like your new resource to implement.
In this case we just leave the default (which is to implement the {\tt
gate.ProcessingResource} interface).
%
\item
The last field selects the directory that you want the new resource created
in. For our example: {\tt z:/tmp}.
\end{itemize}
%
Now we need to compile the class and package it into a JAR file.
%(There's no reason not to use your own favourite alternative, e.g.
%\htlink{http://jakarta.apache.org/ant/}{ANT}.)
%For the pre-requisites of the build process
%that we use (based on Makefiles, and the GNU shell tools) see section
%\ref{sec:howto:build}. When you have these pre-requisites available
%do the following from a command prompt (working from the {\tt
%GoldFish/build} directory that the bootstrapper created for you):
%\\ {\tt
%./configure \\
%make depend \\
%make \\
%make jar \\
%}
The bootstrap wizard creates an \htlink{http://ant.apache.org}{Ant} build file
that makes this very easy -- so long as you have Ant set up properly, you can 
simply run \verb|ant jar|

This will compile the Java source code and package the resulting classes into
{\tt GoldFish.jar}.  If you don't have your own copy of Ant, you can use the
one bundled with GATE - suppose your GATE is installed at
{\tt /opt/gate-5.0-snapshot}, then you can use
{\tt /opt/gate-5.0-snapshot/bin/ant jar} to build.
...
%\begin{itemize}
%\item
%section \ref{sec:api:instant} for how to instantiate the resource from GATE
%Embedded;
%\item
%section \ref{sec:developer:load} for how to load the resource into GATE
%Developer;
%\item
%section \ref{sec:howto:creoleconfig} for how to configure and further
%develop your resource (which will, by default, do nothing!).
%\end{itemize}
%
The default Java code that was created for our GoldFish resource looks like
this:
%
\nsmall
\begin{lstlisting}
...
\begin{figure}[!htb]
\begin{center}
\includegraphics[scale=0.7]{bootstrap2.png}
\end{center}
\caption{BootStrap directory tree}
\label{fig:bootstrap2}
\end{figure}
%
is shown in figure \ref{fig:bootstrap2}. {\tt GoldFish.java} lives in the
{\tt src/sheffield/creole/example} directory.
%{\tt creole.xml} is generated
%in the {\tt build} directory from a source file called {\tt resource.xml}
%which lives in the {\tt src} directory.
{\tt creole.xml} and {\tt build.xml} are in the top {\tt GoldFish} directory.
The {\tt lib} directory is for libraries;
the {\tt classes} directory is where Java class files are placed;
the {\tt doc} directory is for documentation.  These last two, plus
{\tt GoldFish.jar} are created by Ant.

This process has the advantage that it creates a complete source tree and
build structure for the
component, and the disadvantage that it creates a complete source tree and
build structure for the component. If you already have a source tree, you
will need to chop out the bits you need from the new tree (in this case {\tt
GoldFish.java} and {\tt creole.xml}) and copy it into your existing one.
