...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\sect[sec:ontologies:pluginontology]{The Ontology Plugin: Current Implementation}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\sect[sec:ontology:changes5.1]{Ontology API and Implementation Changes}
...
\emph{Before any ontology-related function can be used in GATE, one of the
...
Also, the intended way how ontologies are modeled has been changed:
...
%% \sect[sec:ontology:tipsandfaqs]{Various tipps, tricks and answers to FAQs}



%% TODO: maybe strucutre this a bit better -- list changes in the 
%% API vs. changes in the implementation vs. changes in the 
%% language resources?



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% \sect[sec:howto:ontology]{Miscellaneous how-tos, recommendations and tips}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% TBD.

%% TODO: the whole section needs to get worked over:
%% mention 
%% ontology-enabled JAPE transducing  se:jape:ontologyaware and sec:ontologies:ontology-aware-jape
%% onto root gazetteer,
%% ontology annotation tool
% 

%% THE FOLLOWING already is covered by the main chapter
% Before you can work with ontologies you have to load one of the plugins
% that provide an ontology implementation. Currently there are two such
% plugins included in GATE:
% \begin{itemize}
%  \item \verb!Ontology_OWLIM2!: this plugin provides an implementation that is
% based on Sesame version 1 and OWLIM version 2. This plugin is deprecated but
% kept for backwards compatibility with the ontology implementation the
% was included in GATE prior to version 5.1.
%   \item \verb!Ontology!: this plugin is based on Sesame version 2 and OWLIM 
% version 3. It includes new features, new ontology language resources and
% implements the new methods that were added to the GATE ontology API in 
% version 5.1. This implementation does not support some of the deprecated
% functions in the API any more.
% \end{itemize}
% 
% You can load the desired plugin via the plugin manabger from the File menu
% and then Manage CREOLE plugins. Select either \verb!Ontology OWLIM2! or 
% \verb!Ontology! (recommended) and the ontology language resources provided
% by the plugin will be made available in the language resource menu.
% 
% Note that you cannot use both plugins in the same GATE session!
% 
% In order to make the graphical ontology editor or the ontology annotation
% tool available in GATE you have to also load the \verb!Ontology Tools! 
% plugin prior to loading or creating an ontology.
% 

%% TODO: see if any of this should get merged into the OntoGazetteer 
%% section in the gazetteer chapter
% \paragraph{OntoGazetteer
% 
% 
% The OntoGazetteer is a Processing Resource in which lists of instances of
% ontology concepts can be loaded. For the OntoGazetteer to function properly, it
% needs one or more .lst files, and two .def files, usually called mappings.def,
% and lists.def.
% 
% The .lst file is simply a file with an instance on every line. For instance
% persons.lst will have, on every line, the name of a person.
% 
% The mappings.def file describes the relations between the .lst files and the
% ontology concepts. The format is .lst file:ontology file:ontology concept.
% 
% The lists.def file states the relations between the .lst files and the
% annotation feature the OntoGazetteer should generate. The format is: .lst
% file:feature
% 
% The OntoGazetteer, when run, generates annotations for every instance mentioned
% in the .lst files. All these instances will be annotated with the same
% annotation type, called 'Lookup' ( in the 'default' Annotation Set). Every
% Lookup annotation will have a feature, which is its majorType, which will differ
% per .lst file.
% 
% This is not very useful, because every different concept from the ontology
% will have the same annotation type (namely 'Lookup'). However, since the
% 'majorType' feature differs, we can process them further. And to do that, we
% need a Jape Transducer.
